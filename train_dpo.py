...
import numpy as np
from diffuser.pipeline_with_logprob import pipeline_with_logprob
from diffuser.ddim_with_logprob import ddim_step_with_logprob
import torch
import wandb
...
def sample_with_replacement(dataset, num_samples, initial_weight=1, decay_rate=0.2):
    samples = []
    weights = {key: initial_weight for key in dataset.keys()}
    for _ in range(num_samples):
        ...
        for key, weight in weights.items():
            ...
            if rand_val <= cumulative_weight:
                chosen_key = key
                break
        samples.append(chosen_key)
        weights[chosen_key] *= (1 - decay_rate)    
    return samples
...
def main(_):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="COLM", config=config.to_dict(), init_kwargs={"wandb": {"name": config.run_name}}
        )
    logger.info(f"\n{config}")
    ...
    with open("prompt_set.json", 'r') as f:
        dataset = json.load(f)
    samples = sample_with_replacement(dataset, 1500)
    ...
    global_step = 0
    enhancer = getResponeFromGPT4V
    validators = [getResponeFromGPT4V]
    best_is = -1
    best_fid = 99
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for i in tqdm(
            ...
            # generate prompts
            input = random.choice(list(train_dataset.values()))
            verifying_prompt = prompt['verify'] + '\"' + input + '\"'
            rewrite_prompt = prompt['details'] + '\"' + input + '\"'
            prompts = [enhancer(rewrite_prompt)[0] for _ in range(5)]
            # encode prompts
            prompt_ids = [pipeline.tokenizer(
                ...
                truncation=True,
                max_length=pipeline.tokenizer.model_max_length,
            ).input_ids.to(accelerator.device) for improved_prompt in prompts]
            
            prompt_embeds = [pipeline.text_encoder(prompt_ids1)[0] for prompt_ids1 in prompt_ids]
            ...
            with autocast():
                for prompt_embed in prompt_embeds:
                    ...
                    image_io = BytesIO()
                    images1[0].save(image_io, format='PNG')
                    image_binary = image_io.getvalue()
                    encoded_image = base64.b64encode(image_binary).decode('utf-8')
                    saveBase64(encoded_image)
                    valid_result = [int(accept(validator(prompt["verify"], encoded_image)[0])) for validator in validators]
                    score1 = sum(valid_result)
                    if score1 >= score_w:
                        latents_w, log_probs_w, prompt_embeds_w = latents1, log_probs1, prompt_embed
                    if score1 < score_l:
                        latents_l, log_probs_l, prompt_embeds_l = latents1, log_probs1, prompt_embed

            latents = torch.stack([latents_w,latents_l], dim=1)  # (batch_size, 2, num_steps + 1, 4, 64, 64)
            log_probs = torch.stack([log_probs_w,log_probs_l], dim=1)  # (batch_size, num_steps, 1)
            prompt_embeds = torch.stack([prompt_embeds_w,prompt_embeds_l], dim=1)
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            if epoch!=0 and (epoch+1) % config.n_epochs_per_valid == 0 and accelerator.is_main_process:
                lora_path = ""
                is_score, fid_score = evaluate(config.pretrained.model, lora_path, valid_dataset)
                if is_score > best_is:
                    best_is = is_score
