...
class DDPOTrainer(BaseTrainer):
    ...
    def plot_by_epoch(self, yvalue, title, filename, ylabel):
        mul = len(yvalue) / len(self.reward_list)
        xvalue = [i/mul for i in range(1, len(yvalue) + 1)]
        plt.plot(xvalue, yvalue, 'b', label=ylabel)
        plt.title(title)
        plt.xlabel('Epoch')
        plt.ylabel(ylabel)
        plt.legend()
